...
PartialPolicy Clingo::computePolicy(const std::vector<actasp::AspRule>& goal, double suboptimality) const throw (std::logic_error) {
...
//   cout << "The first kr call took " << (double(kr1_end - kr1_begin) / CLOCKS_PER_SEC) << " seconds" << endl;
  ...
  PartialPolicy policy(allActions);
...
std::vector< AnswerSet > Clingo::computeAllPlans(const std::vector<actasp::AspRule>& goal,
    double suboptimality) const throw(logic_error) {
...
std::list< std::list<AspAtom> > Clingo::query(const std::string &queryString, unsigned int timestep) const throw()  {

  //this depends on our way of representing stuff.
  //iclingo starts from 1, while we needed the initial state and first action to be at time step 0
  unsigned int initialTimeStep = timestep+1;
  unsigned int finalTimeStep = initialTimeStep;
...
void Clingo::resetCurrentState() throw() {
