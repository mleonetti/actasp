...
pos(X,Y+1,I+1) :- north(I), pos(X,Y,I), I=0..n-2.
:- north(I), pos(X,max_y,I).
:- north(I), pos(X,Y,I), obst(X,Y,X,Y+1,I).

pos(X+1,Y,I+1) :- east(I), pos(X,Y,I), I=0..n-2.
:- east(I), pos(max_x,Y,I).
:- east(I), pos(X,Y,I), obst(X,Y,X+1,Y,I).


pos(X-1,Y,I+1) :- west(I), pos(X,Y,I), I=0..n-2.
:- west(I), pos(0,0..max_y,I).
:- west(I), pos(X,Y,I), obst(X,Y,X-1,Y,I).

pos(X,Y-1,I+1) :- south(I),pos(X,Y,I), I=0..n-2.
:- south(I), pos(0..max_x,0,I).
:- south(I), pos(X,Y,I), obst(X,Y,X,Y-1,I).
...
%% pos is inertial
pos(X,Y,I+1) :- pos(X,Y,I), not -pos(X,Y,I+1), I=0..n-2.


obst(X2,Y2,X1,Y1,I) :- obst(X1,Y1,X2,Y2,I).

%obstacles that are always there
obst(1,Y,2,Y,I) :- Y=1..3, I=0..n-1.
obst(X,3,X,4,I) :- X=3..4, I=0..n-1.

%if at some point you know about an obstacle, it doesn't go away
obst(X,Y,Z,K,I+1) :- obst(X,Y,Z,K,I), not -obst(X,Y,Z,K,I),  I=0..n-2.
